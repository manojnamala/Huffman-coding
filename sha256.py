...
def sha256(message):
    ...
    for i in range(8):
        padded.append((bit_length >> (56 - i * 8)) & 0xFF)
    ...
    for chunk_start in range(0, len(padded), 64):
        ...
        w = [0] * 64
        # First 16 words from chunk (big-endian)
        for i in range(16):
    ...
    # Combine hash values into final digest
    return ''.join(f'{x:08x}' for x in h)

# Test the implementation with a known input
if __name__ == "__main__":
    test_input = "abc"
    test_hash = sha256(test_input)
    print(f"SHA-256('abc') = {test_hash}")